...
export default function FillSOPPage() {
  ...
  const evaluateComparison = (field: FormField, value: string) => {
    if (!field.comparisonOperator || field.comparisonOperator === 'none' || !field.comparisonValue) return true
    const target = Number(field.comparisonValue)
    ...
    for (const field of template.fields as FormField[]) {
      ...
      if (field.type === 'file_upload' || field.type === 'image') {
        ...
        if (!val || (typeof val === 'string' && !val.trim())) {
          ...
          errs.push(`"${field.label}" must be a valid number`)
        } else if (!evaluateComparison(field, String(val))) {
          errs.push(`"${field.label}" must satisfy the configured rule`)
        }
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      setErrors(['Unable to save draft right now. Please try again.'])
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {successMessage && (
        <div className="bg-green-50 border border-green-200 rounded p-4 mb-4 text-sm text-green-700">
          {successMessage}
        </div>
      )}
            ...
            {field.type === 'section_header' ? (
              ...
              <>
                ...
                {(field.type === 'number' || field.type === 'currency') && (
                  <input type="number" step="0.01" value={responses[field.id] || ''} onChange={e => updateResponse(field.id, e.target.value)} className="w-full border rounded px-3 py-2 focus:border-primary-500 outline-none" placeholder={field.type === 'currency' ? '0.00' : '0'} />
                )}

                {field.type === 'phone_number' && (
                  <input type="tel" value={responses[field.id] || ''} onChange={e => updateResponse(field.id, e.target.value)} className="w-full border rounded px-3 py-2 focus:border-primary-500 outline-none" placeholder="+91 98765 43210" />
                )}
