...
export default function EditTemplatePage() {
  ...
  useEffect(() => {
    const loadTemplate = async () => {
      try {
        const [deptsRes, usersRes, templateRes] = await Promise.all([
        ...
        setIsDaily(Boolean(template.isDaily))
        setApproverIds(Array.isArray(template.approverIds) ? template.approverIds : [])

        const loadedFields = Array.isArray(template.fields)
        ...
        setFields(loadedFields.length > 0 ? loadedFields : [{ id: generateFieldId(), type: 'short_text', label: '', required: false }])
      } catch (error) {
        console.error('Failed to load template for edit page', error)
        setFields([{ id: generateFieldId(), type: 'short_text', label: '', required: false }])
      } finally {
        setLoading(false)
      }
    }
    ...
    try {
      const res = await fetch(`/api/templates/${params.id}`, {
        ...
        body: JSON.stringify({
          ...
          isDaily,
          fields,
          approverIds: approverIds.length > 0 ? approverIds : null,
        }),
      })
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">Edit SOP Template</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={saving || fields.length === 0} className="bg-primary-600 text-white px-6 py-2 rounded hover:bg-primary-700 disabled:opacity-50">
